...
def NAMIX(pdbfile:str,restrin_file:str=None,A:modnuc=A_no_mod, C:modnuc=C_no_mod, G:modnuc=G_no_mod, T:modnuc=T_no_mod, U:modnuc=U_no_mod, Prefix:str ="",overwrite = False,min=False) -> None:
    ...
    if restrin_file:
        if restrin_file[0].endswith(".pb"):
            ...
            if len(restrin_file) == 4:
                restraints_from_road(restrin_file[0],restrin_file[1],dir_path,min,restrin_file[2],restrin_file[3])
            else: restraints_from_road(restrin_file[0],restrin_file[1],dir_path,min,restrin_file[2])
    if not min:
